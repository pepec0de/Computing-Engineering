...
TestOrdenacion::TestOrdenacion()
{
	...
	/**** EXAJUN21****/
	nombreAlgoritmo.push_back("Junio21");
} 
TestOrdenacion::~TestOrdenacion() {}
...
double TestOrdenacion::ordenarArrayDeInt(int v[],int size,int metodo) 
{
	...
	switch (metodo){
		...
		case SELECCION: estrategia.ordenaSeleccion(v, size);
			...
			/**** EXAJUN21****/
		case JUNIO21:
			estrategia.ordenaJunio21(v, size);
			break;
	}
...
void TestOrdenacion::casoMedio(int metodo)
{
	...
	case 'S': {
		...
		Graficas g;
		/**** EXAJUN21**** -> JUN21 en el caso medio también es de orden cuadrado asi que no se modifica nada en este metodo */
...
void TestOrdenacion::casosTeoricos(int metodo) {
	...
	case JUNIO21:
		for (int talla = tallaIni; talla <= tallaFin; talla += incTalla) {
			...
			// Caso peor: vector ordenado
			tiempoPeor = 9 * ((double)talla / 2);
			for (int i = 0; i <= (talla / 2) - 1; i++) {
				tiempoPeor += 28;
				tiempoPeor += 12 * ((double)talla - 2 - 2 * (double)i);
				tiempoPeor += talla - 2 *(double)i - 1;
			}
