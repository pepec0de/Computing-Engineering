...
void TAlmacen::ListarListaEnvios(Cadena Nomtienda) {
	if(Envios.longitud() > 0) {
		...
		cout << "Precio total de los envíos mostrados: " << PrecioEnvios(Nomtienda) << " euros.\n";
	} else {
        cout << "No hay envíos.\n";
	}
}
...
/// PRACTICA 2 MOD
// Calculará el precio total de los productos que se  van  a  enviar  a  una  tienda  cuyo nombre
// es  pasado  por  parámetro.  Si el  método  recibe una  cadena  vacía,  calculará el precio total
// de todos los envíos pendientes a enviar, independientemente de la tienda.
float TAlmacen::PrecioEnvios(Cadena Nombtienda) {
    float result = 0;
    TPedido envio;
    int pos;
    for (int i = 1; i <= Envios.longitud(); i++) {
        envio = Envios.observar(i);
        if (strcmp(envio.Nomtienda, Nombtienda) == 0 || strcmp(Nombtienda, "") == 0) {
            pos = BuscarProducto(envio.CodProd);
            if (pos != -1) {
                result += envio.CantidadPed*ObtenerProducto(pos).Precio;
            }
        }
    }
    return result;
}
// Bloquear  los  pedidos  realizados por una  determinada  tienda.  Este  bloqueo  no  significa
// que  los  productos tengan  que  reponerse  en  el almacén,  sino  que  temporalmente  son
// eliminados  de  la  Cola  de  pedidos  y almacenados, en  el  mismo  orden,en  una  listaPedidosBloqueados.
// El  método  devolverá  el  número  de pedidos que han sido bloqueados.
int TAlmacen::BloquearPedidos(Cadena Nombtienda) {
    int result = 0;
    TPedido pedido;
    int longitud = Pedidos.longitud();
    for (int i = 0; i < longitud; i++) {
        pedido = Pedidos.primero();
        if (strcmp(pedido.Nomtienda, Nombtienda) == 0) {
            result++;
            PedidosBloqueados.anadirDch(pedido);
        } else {
            Pedidos.encolar(pedido);
        }
        Pedidos.desencolar();
