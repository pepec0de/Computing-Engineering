...
for img = 1:3
    ...
    for i = 1:N
        for j = 1:M
            if IbinRojo(i, j)
                I_deteccion(i, j, :) = Colores(1, :);
            elseif IbinVerde(i, j)
                I_deteccion(i, j, :) = Colores(2, :);
            end
    ...
    for e = 1:n_fresas
        ...
        region_rojo = IbinRojo(yStart : yStart + height, xStart : xStart + width);
        region_verde = IbinVerde(yStart : yStart + height, xStart : xStart + width);
        n_pix_rojos = sum(region_rojo(:));
        % DUDA: así o con sum(IbinFresa(:))
        n_pix_total = n_pix_rojos + sum(region_verde(:));
        
        Ipred_crop = region_rojo + (region_verde .* 2);
        disp(unique(Ipred_crop));
